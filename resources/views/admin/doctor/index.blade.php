...
<div class="dataOverviewSection mt-3">
    <div class="section-title">
        <h6 class="fw-bold m-0">All Doctor<span class="fw-normal text-muted"></span></h6>
        <a href="{{ url('doctor_create') }}" class="primary-btn addBtn">+ Add
            Doctor</a>
    </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.data,function(index, value){
                    let action = `<td><a href='{{url('doctor_edit/`+value.id+`')}}' class='btn btn-warning'>Edit</a> | <a href='javascript::void(0)' onclick='deleteDoctor(`+value.id+`)' class='btn btn-danger'>Delete</a></td>`
                    tdData += `<tr>
                                <td>` + (index + 1) + `</td>
